...
public class StrawStatueModelPartsScreen extends ArmorStandTickBoxScreen<PlayerModelPart> {
    ...
    protected AbstractWidget makeTickBoxWidget(ArmorStand armorStand, int buttonStartY, int index, PlayerModelPart option) {
        ...
        return new TickBoxButton(this.leftPos + 96, this.topPos + buttonStartY + index * 22, 6, 76, option.getName(),
                () -> strawStatue.isModelPartShown(option), (Button button) -> {
    ...
    protected void init() {
        super.init();
        this.refreshProfileButton = this.addRenderableWidget(
                new SpritelessImageButton(this.leftPos + 3, this.topPos + 31, 10, 10, 0, 0, 10,
                        UNSEEN_NOTIFICATION_SPRITE, 10, 10, button -> {
                    StrawStatues.NETWORK.sendToServer(
                            new C2SStrawStatueSetProfileMessage(this.name.getValue().trim(),
                                    true
        ...
        ((StrawStatue) this.getHolder().getArmorStand()).getProfile()
                .map(ResolvableProfile::gameProfile).ifPresent(this::setGameProfile);
    }
